                                    ...
                                    <tbody>
                                        ...
                                        while ($row = mysqli_fetch_assoc($resA)) {
                                            ...
                                            echo '<tr>
                                                    ...
                                                    <td scope="row">';
                                            $sqltt = "SELECT `studentdetails`.`registerNo`, `studentdetails`.`name`, `studentdetails`.`sem`, `studentdetails`.`section`, `secondinternalmarks`.`marks`, `secondinternalmarks`.`studentID` FROM `studentdetails`, `secondinternalmarks` WHERE `secondinternalmarks`.`studentID` = `studentdetails`.`registerNo` AND `studentdetails`.`sem`='{$row['sem']}' AND `studentdetails`.`section`='{$row['section']}' AND `secondinternalmarks`.`subject`='{$row['subjectName']}'";
                                            $restt = mysqli_query($conn, $sqltt);
                                            if ($restt) {
                                                if (mysqli_num_rows($restt) > 0) {
                                                    echo '<a class="btn btn-primary mr-2" href="view_internals_marks.php?operation=secondInternals&sem=' . $row['sem'] . '&sec=' . $row['section'] . '&subject=' . $row['subjectName'] . '">View Marks</a>';
                                                } else {
                                                    echo '<a class="btn btn-primary mr-2" href="enter_internals_marks.php?type=enter_marks&operation=secondInternals&sem=' . $row['sem'] . '&sec=' . $row['section'] . '&subject=' . $row['subjectName'] . '">Enter Marks</a>';
                                                }
                                            }
                                            echo '</td>
                                                </tr>';
